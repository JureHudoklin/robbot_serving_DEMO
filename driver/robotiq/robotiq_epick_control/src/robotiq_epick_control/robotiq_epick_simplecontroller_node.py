...
def genCommand(char, command):
    ...
    if char == 'mo':
        desired_vacuum = 100-60
        command.rMOD = 1
        command.rGTO = 1 
        command.rATR = 0
        command.rPR = desired_vacuum #Maximum vacuum
        command.rSP = 10 # Action Timeout
        command.rFR = desired_vacuum +5 #Minimum vacuum. When to turn on the pump
